...
Version 0.1.0 is intentionally not Authenticode-signed. Its installer filename must include `UNSIGNED`, and Windows can display an unknown publisher warning. A self-signed certificate is not used because it would not establish public publisher trust.
...
If a safe and publicly trusted signing identity becomes available in the future, optional Authenticode support requires Launcher, Setup, the final installer and its embedded uninstaller to use and verify the same signer. That optional path is not required for version 0.1.0.
