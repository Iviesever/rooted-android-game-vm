...
public sealed class FileWorkspaceViewModel(WorkstationOperation run) : ObservableState
{
    ...
    public bool CanUpload => CanChangeScope && _root is { Accessible: true, Writable: true } && _directoryRef is not null;
    ...
    public FileTransferHistoryRow? SelectedTransfer { get => _selectedTransfer; set { Set(ref _selectedTransfer, value); Changed(nameof(CanResumeSelected)); } }
    ...
    public async Task RefreshRootsAsync()
    {
        ...
        Roots = data.GetProperty("roots").Deserialize<FileRootDescriptor[]>(DebugJson.Options)!;
    ...
    {
        if (!root.Accessible) { Message = root.Title + "：" + (root.Reason ?? "不可访问"); return; }
        FileReferences.Relative(relative);
        if (remember && _root is not null) _back.Push((_root, _relative));
        _generation++; var generation = _generation; _root = root; _relative = relative; _directoryRef = null; _cursor = ""; Entries.Clear(); Selection.Clear();
        Address = root.DisplayPath.TrimEnd('/') + (relative.Length == 0 ? "" : "/" + relative); CapabilitiesChanged();
    ...
    public Task RefreshDirectoryAsync() => _root is null ? Task.CompletedTask : NavigateAsync(_root, _relative, false);
    ...
    public DebugRequest UploadRequest(IEnumerable<string> paths) => DebugRequest.Create("files.transfer.plan", new
    { direction = "upload", sources = paths.Select(path => new { localPath = Path.GetFullPath(path) }).ToArray(), destination = new { entryRef = _directoryRef } });
    ...
    public async Task ResumeAsync(FileTransferHistoryRow transfer)
    {
        ...
        finally { IsBusy = false; await RefreshTransfersAsync(); SelectedTransfer = Transfers.FirstOrDefault(item => item.PlanId == transfer.PlanId); }
    }
