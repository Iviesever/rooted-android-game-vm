...
if (args.Length < 4 || args[0] is not ("sample" or "audit"))
    throw new ArgumentException("sample|audit <dataRoot> <output.ndjson> <stage.txt> [seconds=600] [intervalMs=1000|100] [program directories...] | buffers <directory>");
var audit = args[0] == "audit";
var paths = InstallPaths.FromProductRoot(args[1]);
var seconds = args.Length > 4 ? int.Parse(args[4]) : 600;
var interval = args.Length > 5 ? int.Parse(args[5]) : audit ? 100 : 1000;
if (seconds is < 1 or > 7200 || interval < (audit ? 50 : 250) || interval > 10000) throw new ArgumentException("采样时长/间隔超限。");
...
var sampler = new ProcessMemorySampler(paths, directories, audit ? TimeSpan.Zero : null); string? previousStage = null;
try
{
