...
全产品审计还包含同 SDK 下的 netsimd、crashpad 等 `emulator-helper-shared` 辅助进程，以及指定程序目录内的 Setup。枚举合并为单次 WMI 查询；路径仍逐个精确核对。`observedWorkingSetSumBytes` 是本次观测到的进程 WS 之和，共享页可能重复计入；它不是未采样时刻的硬峰值保证，存在错误/退出或清单刷新间隙时不能宣称完整覆盖。`observedPrivateCommitSumBytes` 单独列示，不当作物理 RAM。
