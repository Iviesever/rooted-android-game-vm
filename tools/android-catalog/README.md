...
This project is product-owned metadata code, not an application plugin. It reads Android package, user, volume and file metadata in a short-lived `app_process` process. Requested file hashes stream file contents without returning them. It does not install an APK, change the system image, start listed applications, or write application files.
