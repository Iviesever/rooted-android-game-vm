## 2026-09-21：C真实GUI闭环与v0.5.1交付范围

用户确认最终以v0.5.1完成全部目标，保留旧v0.5.0标签和草稿。源码修复已分批提交：c84bfcf修正小文件单位和本地时间；a229eb3修正活动历史状态与选择保持；1c48962修正目录Enter事件路由。对应CI35585108935、35585810896、35587037373均成功；404项非实机、完整构建、格式检查通过。版本更新后的候选仍须完成自己的构建与交付门禁。

| C要求 | 真实证据与结果 |
| --- | --- |
| 通用应用发现及用户映射 | 名称搜索、应用管理进入文件工作台、真实图标、同名包名消歧；QA用户10的DE可读，切换锁定CE后清空旧目标并禁用上传。未手写包名 |
| 私有文件夹双向传输 | GUI上传5项并导出，两个文件SHA一致，中文路径和空目录保持；首次离线失败保留原计划，GUI明确续作成功，旧guest工具清理确认 |
| 键盘与多选 | 修复后Enter、上一级、后退、面包屑实测通过；Ctrl+A选择文件与目录两个来源，实际导出4项，SHA和空目录保持 |
| 覆盖冲突 | GUI选“备份后覆盖”，实际目标为新SHA、原备份仍为旧SHA，私有文件属主和权限正确 |
| 取消与明确续作 | 122项目录计划在核验阶段被GUI取消，job与账本均cancelled；GUI继续原plan完成122项。121文件随后通过独立下载与本机原文件逐项比对。切出再回文件页保留取消入口。该次是复制前取消，不冒称部分复制后取消；大文件分块取消证据复用D/B既有验收 |
| 真实拖放与缺失根 | 用户完成Explorer跨窗拖放，后台新建第二测试应用的OBB根；47B文件SHA一致，目录10213:1079:2770、文件10213:1079:660，界面刷新为可浏览可导出 |
| 多根应用导出与状态 | GUI选择私有和外部两根，158项/136文件/22目录/67638426字节全部核验。活动历史显示执行中，刷新不回退旧失败状态，结束显示已核验，本地时间正确 |
| 人与AI同一服务 | GUI选应用A时，CLI独立发现应用B、取得引用并导出其私有文件，SHA与GUI上传结果一致；结果分页100到122项、错误摘要、备份和暂存入口均有真实界面证据 |

本机逐项证据为gui-private-folder-roundtrip、gui-multiselect-export、gui-overwrite、gui-cancel-resume、gui-resumed-directory-independent-roundtrip、gui-drag-creatable-obb、gui-multiple-app-roots-export、gui-ai-equivalence及gui-directory-keyboard-navigation等acceptance文件。用户只协助了首个文件夹选择器、一次跨窗拖放和最后保存按钮；其余界面操作与数据核验由代理完成。工具的原生控件缓存/前台截图及跨窗口拖动限制不被当成产品通过证据，失败尝试分别保留。

仍保留已有边界：本机出现过短暂ADB离线和有限元数据重读，失败不自动重放写入，已验证从原任务恢复；不宣称瞬断原因已消除。App业务导入、声学传播精度等排除项没有重新宣称通过。E最终精确安装包、生产备份覆盖、装后关键回归和公开发布尚未完成，整个goal未完成。
## 2026-09-21：真实结果界面与多批上传规划修复

GUI候选已实际验证停机历史读取、键盘选择失败任务、整批失败原因与逐项状态、结果分页。发现固定无障碍名称遮盖摘要文本，修复提交cb35d9f及CI35579856043成功；真实界面现在可读到失败原因，未执行的条目保持待执行。

为分页建立122项真实只读计划时，发现上传目标分批核对会在枚举原列表时替换其中元素，跨100项或请求字节边界触发集合修改异常。两种边界回归均先复现失败，修复a0b7b68对分批输入取快照；403项非实机、完整构建、格式检查和CI35580497970通过。新候选122项规划实测成功，安卓目标仍不存在，未执行上传。真实GUI从100项加载至122项后禁用下一页；CLI最后22项、无后续页、待执行状态一致。本机证据为gui-result-pagination-acceptance、gui-failed-result-accessibility及gui-plan-batch系列。

本段仅证明以上C/B局部项。原生文件选择器的工具控件缓存/前台窗口一致性问题仍未解决，真实上传/导出、多选、拖放、冲突、取消续作等C操作仍未完整通过；最终生产覆盖和发布E未执行。不能将403测试、CI或本段分页验收等同整个goal完成。最后正常保存停机，无QEMU；实际运行状态仍须重新核查。
# Review 剩余优化执行证据
...
## 2026-09-21 D6：剩余名称、类型、链接与并发组合

remaining-matrix-acceptance.json（run daac14dd）补齐此前明确缺少的六组实机组合：

- 规划后把远端目标父目录换成链接，执行以path_escape拒绝，原目标和链接指向的隔离哨兵均保持。
- 规划后把Windows下载父目录换成junction，执行明确拒绝目录链接；隔离哨兵及预先记录的目录ACL散列均未改变。
- 非空目录覆盖为普通文件，原子树与空目录留在真实备份；通过明确的新计划恢复目录，替换后的普通文件也另存备份。
- 两个不同计划同时争同一目标，第二任务实际排队，第一成功后第二以plan_stale拒绝；目标为第一份内容，原值备份保留。
- 两处来源的相同目标名称在计划中标明target_collision，即使选择overwrite也以plan_has_issues阻止执行且无目标写入。
- Android的Case.txt/case.txt向Windows目录导出时阻止大小写折叠；tar保留两个原名和各自不同内容。

故障只注入新建readfixture子树与本机任务目录。未删除唯一原件、未修改正式资源，也未制造内存或磁盘耗尽。首次测试脚本因Windows PowerShell无法加载ACL模块及输出编码而中断；改用已验证的PowerShell 7继续。另一次断言误用通用错误码，实际传输包装返回transfer_failed且明确说明目录链接；保留原失败响应，另用新微型夹具持久记录ACL前后散列后通过。没有改产品错误处理或放宽保护来迎合测试。

最后由源码CLI正常保存停机，job910601b287c143e88117bbd60ad23f65成功且无QEMU。已安装EXE仍为0.5.0.0，SHA256为197eef36d9816d0ad66f2c41642191252a9306364d8c44a8b6f86eb31eb14726；v0.5.0仍为草稿。A6/B5源码82fab85及CI35521875971成功，397非实机/构建/格式/DEX证据仍适用，本批仅补实机证据。

当前允许的A/B/D后台与CLI范围已有上述对应证据，不能当作完整目标完成。C尚需缺失根上传入口的GUI接入、真实选择器/多选/拖放/键盘/冲突/取消续作/可访问性及与CLI逐项对照；E尚需新版本精确资产、生产备份覆盖、安装目录CLI与GUI回归、正式数据核对和发布。目标文件保留的“暂时只做后台与CLI”限制仍有效，未擅自进入C/E。
